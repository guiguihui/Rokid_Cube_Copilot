...
export function classifyByAnchorsHue(rgb, anchors) {
  ...
  // 白锚：优先取 color==='W' 的锚（有中心白锚时，额外加入的低饱和彩格不会抢走白判定）；
  //       没有则退回"最低饱和"。这样支持把人工确认格并入锚池而不破坏白色判定。
  let whiteIdx = -1;
  for (let i = 0; i < anchors.length; i++) if (anchors[i].color === 'W') { whiteIdx = i; break; }
  if (whiteIdx < 0) { let ws = Infinity; for (let i = 0; i < ah.length; i++) if (ah[i].s < ws) { ws = ah[i].s; whiteIdx = i; } }
  const whiteS = ah[whiteIdx].s; let minColoredS = Infinity;
...
// ★同面内相对细分红/橙★：红/橙色相接近、设备光照下易混（橙常被测成低色相→判成红）。
//   思路：① 优先按本面 9 格里"暖色格"的橙度(g-b)/(r+g+b) 分布——若有明显两簇(间隙大)，
//   就在间隙处切开（低橙度=红、高=橙），这就是"按颜色多寡/分布来识别"；
//   ② 没有明显两簇时，用本魔方的红锚色/橙锚色橙度中点作阈值（缺锚色则用默认参照）。
//   只动 base 判为 R/O 的格子，不影响其它颜色；skip[i]=true 的格子(人工确认)跳过。
const DEF_RED = { r: 200, g: 35, b: 35 }, DEF_ORANGE = { r: 235, g: 120, b: 30 };
function orangeness(c) { return (c.g - c.b) / (c.r + c.g + c.b + 1); }
export function refineRedOrange(cells, redRef, orangeRef, skip) {
  redRef = redRef || DEF_RED; orangeRef = orangeRef || DEF_ORANGE;
  const om = [];
  for (let i = 0; i < cells.length; i++) {
    const col = cells[i].color;
    if ((col === 'R' || col === 'O') && !(skip && skip[i])) om.push({ i, o: orangeness(cells[i]) });
  }
  if (!om.length) return cells;
  om.sort((a, b) => a.o - b.o);
  let gapI = -1, gap = 0;
  for (let k = 1; k < om.length; k++) { const g = om[k].o - om[k - 1].o; if (g > gap) { gap = g; gapI = k; } }
  let thr;
  if (om.length >= 3 && gap >= 0.05 && gapI > 0) thr = (om[gapI].o + om[gapI - 1].o) / 2; // 本面分布间隙(多寡)
  else thr = (orangeness(redRef) + orangeness(orangeRef)) / 2; // 红/橙锚色橙度中点
  for (const x of om) cells[x.i].color = x.o >= thr ? 'O' : 'R';
  return cells;
}
...
export const SAMPLE_REGION = { cx: 0.58, cy: 0.66, side: 0.22 };
...
// ★局部精修定位★：魔方就在固定框(SAMPLE_REGION)附近 → 只在其周围搜索窗内，
// 按"贴纸度"做行/列一维投影、取半峰平台边缘 → 紧贴的方框。不靠黑缝、不靠色相多样性
// （很多魔方无缝、有的面同色），同色面也能给实心平台照样精修。
// 返回 {x0,y0,side}(方形,全分辨率像素) 或 null（调用方回退固定框）。
// opt.debug=true 时失败也返回 {rejected,reason,_dbg} 便于 PC 调参。
export const CUBE_CFG = {
  PRIOR_CX: 0.58, PRIOR_CY: 0.66, PRIOR_SIDE: 0.22, // 位置先验中心/边长(占帧)——魔方大致所在；可被 opt 覆盖/优化
  WIN_W: 120,            // 搜索窗内工作列数（≈9k 工作像素，<100ms）
  WIN_EXPAND: 0.5,     // 窗半径 = 此值 × priorSide（结构上排除上方屏幕）
  PRIOR_SIGMA: 0.7,     // 位置先验高斯 σ = 此值 × priorSide（越小越信先验）
  S_STICKER: 0.28, V_MIN: 0.32,   // 鲜艳贴纸门限
  S_WHITE: 0.2, V_WHITE: 0.55,   // 干净白贴纸门限
  SKIN_H0: 10, SKIN_H1: 45, SKIN_S0: 0.15, SKIN_S1: 0.55, SKIN_V0: 0.4, SKIN_PEN: 0.08, // 肤色带降权(鲜艳红/橙 s>0.55 逃过)
  EDGE_FRAC: 0.45,      // 平台边缘 = 峰值的此比例处
  GRAD_GAIN: 0.5,         // 纹理(梯度)项权重：魔方面贴纸/缝处处高对比，手/桌面平滑→纹理把"暗/白/中间色贴纸"也纳入面范围。0=关
  GRAD_SCALE: 150,      // 纹理归一化尺度(相邻格 |dr|+|dg|+|db| 之和)
  MASS_MIN_FRAC: 0.05,  // 总贴纸质量/窗像素 下限，否则 null
  FILL_MIN: 0.5,       // 平台实心度下限
  SIDE_MIN_REL: 0.3, SIDE_MAX_REL: 1.1, // 检测 side/priorSide 容许区间
  CENTER_MAX_REL: 0.6, // 中心漂移/priorSide 上限
  ASPECT_MAX: 0.9,     // |bw-bh|/max 上限（放宽：手部遮挡致非方时，方化后仍比偏移的固定框好）
};
export function locateCubeLocal(rgba, width, height, prior, opt) {
  const C = { ...CUBE_CFG, ...(opt || {}) };
  const dbg = opt && opt.debug;
  const fail = (reason, d) => (dbg ? { rejected: true, reason, _dbg: d || null } : null);
  const pr = prior || { cx: C.PRIOR_CX, cy: C.PRIOR_CY, side: C.PRIOR_SIDE };
  const P = regionPx(width, height, pr);
  const pcx = P.x0 + P.side / 2, pcy = P.y0 + P.side / 2;
  // 搜索窗（全分辨率像素，夹到帧内）
  const half = C.WIN_EXPAND * P.side;
  const wx0 = Math.max(0, Math.floor(pcx - half)), wx1 = Math.min(width, Math.ceil(pcx + half));
  const wy0 = Math.max(0, Math.floor(pcy - half)), wy1 = Math.min(height, Math.ceil(pcy + half));
  const winW = wx1 - wx0, winH = wy1 - wy0;
  if (winW < 8 || winH < 8) return fail('window');
  const nx = C.WIN_W, ny = Math.max(8, Math.round(C.WIN_W * winH / winW));
  const sx = winW / nx, sy = winH / ny;
  // 可分离位置先验（折进行/列权重表，无 2D exp）
  const sigx = C.PRIOR_SIGMA * (P.side / sx), sigy = C.PRIOR_SIGMA * (P.side / sy);
  const cWx = (pcx - wx0) / sx, cWy = (pcy - wy0) / sy;
  const wxw = new Float64Array(nx), wyw = new Float64Array(ny);
  for (let i = 0; i < nx; i++) { const t = (i - cWx) / sigx; wxw[i] = Math.exp(-t * t); }
  for (let j = 0; j < ny; j++) { const t = (j - cWy) / sigy; wyw[j] = Math.exp(-t * t); }
  // Pass1：采样每个工作格中心像素(RGB + HSV)
  const NN = nx * ny;
  const cr = new Float64Array(NN), cg = new Float64Array(NN), cb = new Float64Array(NN);
  const csA = new Float64Array(NN), cvA = new Float64Array(NN), chA = new Float64Array(NN);
  for (let j = 0; j < ny; j++) {
    const py = Math.min(height - 1, (wy0 + (j + 0.5) * sy) | 0);
    for (let i = 0; i < nx; i++) {
      const px = Math.min(width - 1, (wx0 + (i + 0.5) * sx) | 0);
      const k = (py * width + px) * 4, idx = j * nx + i;
      const r = rgba[k], g = rgba[k + 1], b = rgba[k + 2];
      cr[idx] = r; cg[idx] = g; cb[idx] = b;
      const hsv = rgbToHsv(r, g, b); csA[idx] = hsv.s; cvA[idx] = hsv.v; chA[idx] = hsv.h;
    }
  }
  // Pass2：贴纸度(0/1) + 纹理(梯度) → 行/列投影
  const colProf = new Float64Array(nx), rowProf = new Float64Array(ny);
  let totalMass = 0;
  const GG = C.GRAD_GAIN, GS = C.GRAD_SCALE || 300;
  for (let j = 0; j < ny; j++) {
    const wj = wyw[j];
    for (let i = 0; i < nx; i++) {
      const idx = j * nx + i;
      const s = csA[idx], v = cvA[idx], h = chA[idx];
      const base = ((s >= C.S_STICKER && v >= C.V_MIN) || (s <= C.S_WHITE && v >= C.V_WHITE)) ? 1 : 0;
      let tex = 0;
      if (GG > 0) { // 局部梯度：与右、下邻格的颜色差(贴纸边/缝处高，平滑背景低)→把暗/白贴纸纳入面范围
        let g2 = 0;
        if (i + 1 < nx) { const o = idx + 1; g2 += Math.abs(cr[idx] - cr[o]) + Math.abs(cg[idx] - cg[o]) + Math.abs(cb[idx] - cb[o]); }
        if (j + 1 < ny) { const o = idx + nx; g2 += Math.abs(cr[idx] - cr[o]) + Math.abs(cg[idx] - cg[o]) + Math.abs(cb[idx] - cb[o]); }
        tex = g2 / GS; if (tex > 1) tex = 1;
      }
      let val = base + GG * tex;
      if (val <= 0) continue;
      if (h >= C.SKIN_H0 && h <= C.SKIN_H1 && s >= C.SKIN_S0 && s <= C.SKIN_S1 && v >= C.SKIN_V0) val *= C.SKIN_PEN;
      val *= wxw[i] * wj;
      colProf[i] += val; rowProf[j] += val; totalMass += val;
    }
  }
  if (totalMass < C.MASS_MIN_FRAC * nx * ny) return fail('mass', { totalMass: +totalMass.toFixed(1) });
  // 半峰平台边缘
  const edges = (prof, n) => {
    let peak = 0; for (let i = 0; i < n; i++) if (prof[i] > peak) peak = prof[i];
    if (peak <= 0) return null;
    const thr = C.EDGE_FRAC * peak;
    let lo = -1, hi = -1;
    for (let i = 0; i < n; i++) if (prof[i] >= thr) { lo = i; break; }
    for (let i = n - 1; i >= 0; i--) if (prof[i] >= thr) { hi = i; break; }
    if (lo < 0 || hi <= lo) return null;
    let sum = 0; for (let i = lo; i <= hi; i++) sum += prof[i];
    return { lo, hi, fill: sum / ((hi - lo + 1) * peak) };
  };
  const ex = edges(colProf, nx), ey = edges(rowProf, ny);
  if (!ex || !ey) return fail('edge', { totalMass: +totalMass.toFixed(1) });
  // 回全分辨率 + 取方
  const bx0 = wx0 + ex.lo * sx, bx1 = wx0 + (ex.hi + 1) * sx;
  const by0 = wy0 + ey.lo * sy, by1 = wy0 + (ey.hi + 1) * sy;
  const bw = bx1 - bx0, bh = by1 - by0;
  const bcx = (bx0 + bx1) / 2, bcy = (by0 + by1) / 2;
  let side = Math.round((bw + bh) / 2);
  const sideRel = side / P.side, drift = Math.hypot(bcx - pcx, bcy - pcy) / P.side;
  const aspect = Math.abs(bw - bh) / Math.max(bw, bh);
  const _dbg = { totalMass: +totalMass.toFixed(1), fillX: +ex.fill.toFixed(2), fillY: +ey.fill.toFixed(2), sideRel: +sideRel.toFixed(2), drift: +drift.toFixed(2), aspect: +aspect.toFixed(2) };
  if (Math.min(ex.fill, ey.fill) < C.FILL_MIN) return fail('fill', _dbg);
  if (sideRel < C.SIDE_MIN_REL || sideRel > C.SIDE_MAX_REL) return fail('side', _dbg);
  if (drift > C.CENTER_MAX_REL) return fail('drift', _dbg);
  if (aspect > C.ASPECT_MAX) return fail('aspect', _dbg);
  side = Math.min(side, width, height);
  const x0 = Math.max(0, Math.min(width - side, Math.round(bcx - side / 2)));
  const y0 = Math.max(0, Math.min(height - side, Math.round(bcy - side / 2)));
  return { x0, y0, side, _dbg };
}
