...
export const SAMPLE_REGION = { cx: 0.55, cy: 0.62, side: 0.30 };
...
export const CUBE_CFG = {
  WIN_W: 96,            // 搜索窗内工作列数（≈9k 工作像素，<100ms）
  WIN_EXPAND: 0.75,     // 窗半径 = 此值 × priorSide（结构上排除上方屏幕）
  PRIOR_SIGMA: 0.6,     // 位置先验高斯 σ = 此值 × priorSide（越小越信先验）
  S_STICKER: 0.40, V_MIN: 0.25,   // 鲜艳贴纸门限
  S_WHITE: 0.20, V_WHITE: 0.65,   // 干净白贴纸门限
  SKIN_H0: 10, SKIN_H1: 45, SKIN_S0: 0.15, SKIN_S1: 0.55, SKIN_V0: 0.40, SKIN_PEN: 0.15, // 肤色带降权(鲜艳红/橙 s>0.55 逃过)
  EDGE_FRAC: 0.60,      // 平台边缘 = 峰值的此比例处
  MASS_MIN_FRAC: 0.06,  // 总贴纸质量/窗像素 下限，否则 null
  FILL_MIN: 0.55,       // 平台实心度下限
  SIDE_MIN_REL: 0.45, SIDE_MAX_REL: 1.30, // 检测 side/priorSide 容许区间
  CENTER_MAX_REL: 0.45, // 中心漂移/priorSide 上限
  ASPECT_MAX: 0.35,     // |bw-bh|/max 上限
};
export function locateCubeLocal(rgba, width, height, prior, opt) {
  ...
  const P = regionPx(width, height, prior);
  ...
  for (let j = 0; j < ny; j++) {
    ...
    for (let i = 0; i < nx; i++) {
      ...
      const hsv = rgbToHsv(rgba[k], rgba[k + 1], rgba[k + 2]);
      const s = hsv.s, v = hsv.v, h = hsv.h;
      if (!((s >= C.S_STICKER && v >= C.V_MIN) || (s <= C.S_WHITE && v >= C.V_WHITE))) continue;
      let c = 1;
      if (h >= C.SKIN_H0 && h <= C.SKIN_H1 && s >= C.SKIN_S0 && s <= C.SKIN_S1 && v >= C.SKIN_V0) c = C.SKIN_PEN;
      const val = c * wxw[i] * wj;
